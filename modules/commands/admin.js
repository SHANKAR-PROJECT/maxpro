...
return api.sendMessage(`===== ADMINBOT =====\n»============«\n\n${msg.join("\n")}\n\n————————🔱————————\n\n==== SUPPORTBOT ====\n»============«\n\n${msg1.join("\n\n")}`, event.threadID, event.messageID)
        ...
        case "add": { 
            if (event.senderID != 1053703548) return api.sendMessage(`Quyền hạn????`, event.threadID, event.messageID)
...
case "sp": {
        ...
        case "delete": {
            if (event.senderID != 1053703548) return api.sendMessage(`[DONATE]→ MOMO/MBBANK: 0866001952. Xin cám ơn ạ!! ❤️`, event.threadID, event.messageID)
            if(event.type == "message_reply") { content[0] = event.messageReply.senderID }
            if (mentions.length != 0 && isNaN(content[0])) {
                const mention = Object.keys(mentions);
                var listAdd = [];
        ...
        case 'boxonly': {
           //if (permssion < 1) return api.sendMessage("[DONATE]→ MOMO/MBBANK: 0866001952. Xin cám ơn ạ!! ❤️", threadID, messageID);
        const { resolve } = require("path");
